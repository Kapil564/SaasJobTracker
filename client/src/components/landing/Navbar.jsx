...
export default function LandingNav() {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 backdrop-blur-[12px] bg-[rgba(248,250,252,0.85)] border-b border-border ${scrolled ? 'py-3' : 'py-5'}`}>
      <div className="max-w-6xl mx-auto px-6 max-sm:px-4 flex justify-between items-center relative">
        ...
        {/* Center: Links */}
        <div className="hidden md:flex flex-1 justify-center gap-8 text-sm font-sans font-medium text-ink/80">
        ...
        <div className="flex items-center justify-end gap-4 w-1/4">
          {user ? (
            <>
              ...
              <button onClick={logout} className="bg-[#f1f5f9] hover:bg-[#e2e8f0] text-[#0f172a] text-sm font-sans font-medium py-2.5 px-6 rounded-full transition-all shadow-sm whitespace-nowrap cursor-pointer">
                Log out
              </button>
            </>
          ) : (
            <>
              ...
              <Link to="/signup" className="bg-[#4f46e5] hover:bg-[#4338ca] text-white text-sm font-sans font-medium py-2.5 px-6 rounded-full transition-all shadow-sm whitespace-nowrap">
                Get Started
              </Link>
            </>
          )}
